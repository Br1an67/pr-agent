...
class LiteLLMAIHandler(BaseAiHandler):
    ...
    async def chat_completion(self, model: str, system: str, user: str, temperature: float = 0.2, img_path: str = None):
        try:
            ...
            _model_name = kwargs.get("model", "unknown")
            _api_base = kwargs.get("api_base", "unknown")
            _ai_timeout = kwargs.get("timeout", "unknown")
            get_logger().info(f"LLM call starting: model={_model_name}, api_base={_api_base}, timeout={_ai_timeout}")
            _llm_t0 = _time_mod.time()
            resp, finish_reason, response_obj = await self._get_completion(**kwargs)
            _llm_elapsed = _time_mod.time() - _llm_t0
            get_logger().info(
                f"LLM call done in {_llm_elapsed:.1f}s, finish_reason={finish_reason}, resp_len={len(str(resp))}"
            )
