...
class PRReviewer:
    ...
    def __init__(
        ...
        """
        get_logger().info(f"PRReviewer.__init__: creating git_provider for {pr_url}")
        self.git_provider = get_git_provider_with_context(pr_url)
        get_logger().info(f"PRReviewer.__init__: git_provider created")
        self.args = args
        self.incremental = self.parse_incremental(args)  # -i command
        get_logger().info(f"PRReviewer.__init__: incremental parsed")
        if self.incremental and self.incremental.is_incremental:
            self.git_provider.get_incremental_commits(self.incremental)

        get_logger().info(f"PRReviewer.__init__: getting main language")
        self.main_language = get_main_pr_language(self.git_provider.get_languages(), self.git_provider.get_files())
        get_logger().info(f"PRReviewer.__init__: main_language={self.main_language}")
        self.pr_url = pr_url
        ...
        get_logger().info(f"PRReviewer.__init__: creating ai_handler")
        self.ai_handler = ai_handler()
        self.ai_handler.main_pr_language = self.main_language
        get_logger().info(f"PRReviewer.__init__: ai_handler created")
        self.patches_diff = None
        self.prediction = None
        answer_str, question_str = self._get_user_answers()
        get_logger().info(f"PRReviewer.__init__: getting pr_description")
        self.pr_description, self.pr_description_files = self.git_provider.get_pr_description(
            split_changes_walkthrough=True
        )
        get_logger().info(f"PRReviewer.__init__: pr_description obtained")
        if (
        ...
        get_logger().info(f"PRReviewer.__init__: building vars dict")
        self.vars = {
    ...
    async def run(self) -> None:
        try:
            get_logger().info(f"PRReviewer.run() STARTED for {self.pr_url}")
            ...
            get_logger().info("PRReviewer: calling retry_with_fallback_models")
            await retry_with_fallback_models(self._prepare_prediction, model_type=ModelType.REGULAR)
            get_logger().info(f"PRReviewer: prediction done, has_prediction={bool(self.prediction)}")
    ...
    async def _prepare_prediction(self, model: str) -> None:
        get_logger().info(f"_prepare_prediction: getting PR diff with model={model}")
        ...
        if self.patches_diff:
            get_logger().info(f"PR diff obtained, length={len(self.patches_diff)}")
            get_logger().info(f"Calling _get_prediction with model={model}")
            self.prediction = await self._get_prediction(model)
            get_logger().info(f"_get_prediction completed, response_length={len(str(self.prediction))}")
        else:
